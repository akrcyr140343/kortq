...
> **อัปเดต: ระบบคิวเกม Q1–Q3** — "เกมถัดไป" (Next Up) เดียวถูกขยายเป็นคิวเกมสูงสุด 3 คิว (`session.gameQueue`) และไม่มีการจัดลงคอร์ตหมายเลขอีกต่อไป
> - Fair ใช้ได้เฉพาะตอนจัดแต่ละ Q (สร้างใหม่ หรือจับแฟร์ใหม่ใน Q เดิม) และไม่คำนวณใหม่อัตโนมัติตอนส่ง; ผู้เล่นที่อยู่ใน Q อื่นหรือกำลังเล่นไม่ถูกเลือก
> - จุดที่ `fairSkips` เปลี่ยน ย้ายจาก `startGame` ไปเป็น **ตอนเรียก Q1 ลงสนาม (`sendFirstQueuedGame`)** ซึ่งเริ่มจับเวลาทันที: 4 คนที่ลง reset = 0, คนที่ว่างจริง (ไม่อยู่ใน Q ใดและไม่ได้เล่น) +1
> - fairLogs ใช้รูปแบบเดิม (`target: "nextup"`, `eventKind: "fair-staging"`) — Firestore rules ไม่ต้องแก้
> - เอกสารส่วนด้านล่างที่พูดถึง "Court Fair", "Next Up" และ "เริ่มเกม" ให้อ่านเป็น "Q" และ "เรียกลงสนาม" ตามลำดับ
...
- "จับแฟร์" ครั้งแรก (Next Up ยังว่าง) = ชุด Fair อันดับดีที่สุด; "จับแฟร์ใหม่" เมื่อมีชุด staged แล้ว = เลื่อน cursor ไปชุดอันดับถัดไปจากชุดปัจจุบันในกลุ่มทางเลือกที่ engine จัดไว้ (alternativeCount+1) แล้ววนกลับชุดแรกเมื่อครบ; ถ้ามีทางเลือกเดียว (เช่น forced ครบ 4) คงชุดเดิม — forced/overdue อยู่ครบทุกชุด และทุกชุดถูกแบ่งทีมใหม่ด้วย teammate/opponent-repeat + skill guard เดิม (ไม่ reuse การแบ่งจากชุดก่อน)
- Reroll ไม่เปลี่ยน `fairSkips` ไม่ว่าถูกเลือกหรือหลุด — ไม่มีการนับ reroll เป็น skip
